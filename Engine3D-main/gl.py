...
class Renderer(object):
    ...
    def glTriangle_bc(self, A, B, C, texCoords = (), normals = (), verts = (), color = None):
        ...
        triangleNormal = ml.norm(triangleNormal)
                    ...
                    if 0 <= x < self.width and 0 <= y < self.height:
                        if z < self.zbuffer[x][y] and z<=1 and z >= -1:
                            ...
                            if self.active_shader:
                                ...
                                self.glPoint(x,y, _color( r, g, b) or self.curr_color )


    def glTransform(self, vertex, vMatrix):
        augVertex = V4(vertex[0], vertex[1], vertex[2], 1)
        transVertex = ml.vecMatrix(augVertex, vMatrix)
    ...
    def glCamTransform(self, vertex):
        augVertex = V4(vertex[0], vertex[1], vertex[2], 1)
        #ransVertex = self.viewPortMatrix @ self.proyectionMatrix @ self.viewMatrix @ augVertex
        transVertex = ml.matrixMultiplication(self.viewPortMatrix, self.proyectionMatrix)
        transVertex2 = ml.matrixMultiplication(transVertex, self.viewMatrix)
        transVertex3 = ml.vecMatrix(augVertex ,transVertex2)
    ...
    def glCreateRotationMatrix(self, rotate=V3(0,0,0)):
        ...
        mult1 = ml.matrixMultiplication(rotationX, rotationY)
        mult2 = ml.matrixMultiplication(mult1, rotationZ)
...
#
        ...
        mult1 = ml.matrixMultiplication(translateMatrix, scaleMatrix)
        mult2 = ml.matrixMultiplication(mult1, rotationMatrix)
    ...
    def glViewMatrix(self, translate = V3(0,0,0), rotate = V3(0,0,0)):
        ...
        self.viewMatrix = ml.inverse(self.camMatrix)


    def glLookAt(self, eye, camPos = V3(0,0,0)):
        forward = ml.subVectors(camPos, eye)
        #print(type(forward))
        forward = ml.norm(forward)

        right = ml.crossProduct(V3(0,1,0), forward)
        right = ml.norm(right)

        up = ml.crossProduct(forward, right)
        up = ml.norm(up)
        ...
        self.viewMatrix = ml.inverse(camMatrix)
        #print(type(self.viewMatrix))
